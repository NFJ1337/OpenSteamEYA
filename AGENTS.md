...
## 触发词：裸回复「2」= 版本 +1 → 打包 → 精简累积日志发布 release → 推送源码（2026-09-17 用户复核）
...
- **版本号只在收到 `2` 时 +1（2026-09-17 用户确认）**：其它任何情况——包括 `1`、用户口头说「打包 / 重新打包」、
  或没提版本号时——都**不自动改动版本号**；只有用户当次明确要求 +1（或给出目标版本号）才 bump。
  （版本号被别人手动 +1 过也属正常，不用据此推断规则。）
...
## 验证时不要弹出软件界面（2026-09-18 用户明确要求）
- 验证改动**不要启动 GUI 程序**（弹窗会打扰用户）：默认只做编译 + 代码/数据层面的核对，
  界面行为如实标注「未实跑验证」，不要为了截图/点按钮把窗口弹出来。
- 确实必须跑界面才能确认时（例如只能靠 UI 复现的问题），**先问用户**，得到同意再跑；
  跑完立刻关闭，并回报「已关闭、无残留进程」。
- 无界面也能验的优先用无界面手段：临时控制台探针、直接读日志/数据文件、算哈希等。
## 改动范围
