...
///  3. 当前数据目录\Languages\*.json（用户拖入新语言，无需重新编译）。
...
internal static class LanguageCatalog
{
    ...
    /// <summary>用户可拖入自定义语言包的目录（当前数据目录\Languages）。</summary>
    public static string UserLanguagesFolder => Path.Combine(AppPaths.DataRoot, LanguagesFolderName);
