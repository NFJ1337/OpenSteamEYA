...
internal sealed class SteamLoginCacheService
{
    ...
    // 用户在缓存账号页的删除/清空会互相覆盖（丢失刚缓存的账号 / 已删账号复活）。临界区全为
    ...
    public string CacheFilePath => Path.Combine(AppFolderPath, CacheFileName);

    public string AvatarFolderPath => Path.Combine(AppFolderPath, AvatarFolderName);
